...
│   ├── .zprofile           # Login shell: Homebrew, NVM, sources .aliases/.functions/.exports
│   ├── .zshrc              # Interactive shell: options, key bindings, Starship prompt
...
- **No personal info in tracked files.** Git user config goes in `~/.gitconfig.local` (generated by setup wizard, included via `[include]` in `.gitconfig`). Secrets go in `~/.secrets` (sourced by `.zprofile`).
...
## Dotfile Loading Order (in .zprofile)
